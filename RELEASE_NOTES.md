...
本版本重点改善首次部署及从无密码版本升级时的管理端体验：用户打开 Web 面板后即可完成管理员密码初始化，无需再手工调用 API 或拼写复杂命令；同时保留 Docker、NAS 与远程访问场景所需的初始化身份校验，避免尚未设置密码的面板被他人抢先接管。

#### 核心变更

1. **🧭 新增 Web 首次使用引导**
   - 检测到尚未配置管理员密码时，登录页自动切换为“首次使用 · 创建管理员密码”界面；
   - 在页面中直接输入新密码与确认密码，设置成功后自动建立管理会话并进入控制台；
   - 已经设置过密码的实例仍显示普通登录界面，不会重复触发初始化流程；
   - 从没有管理员密码的旧版本升级时保留频道、Cookie、推流模式等现有配置，不要求重建配置文件。

2. **🔐 为 Docker / NAS / 局域网初始化增加一次性验证码**
   - 服务在“尚未设置密码”的每次进程启动中生成一枚 16 位高熵随机验证码，并按四组字符显示在启动日志中；
   - Docker、NAS、局域网 IP 与反向代理访问必须同时提交最新验证码，防止同网段访客抢先创建管理员密码；
   - 验证码只在内存中保存 SHA-256 摘要，设置成功立即作废，服务重启后旧码也自动失效；
   - 连字符和字母大小写不影响识别；同一来源连续输错 5 次会锁定 10 分钟，降低暴力猜测风险。

3. **🏠 简化可信本机首次设置**
   - 在运行主机上通过 `localhost`、`127.0.0.1` 或 IPv6 回环地址直连时，可直接创建密码而无需抄写验证码；
   - 一旦请求包含代理转发头，或使用局域网/公网主机名访问，仍会强制要求日志中的一次性验证码；
   - 初始化接口增加专用请求头、请求体大小限制和严格格式校验，避免被普通跨站表单误触发。
...
4. **🔑 保持管理登录与 Jellyfin 播放完全解耦**
   ...
   - 已配置密码和播放令牌的现有实例不轮换令牌，原 M3U 地址保持有效；只有从无播放令牌旧版本升级或忘记密码后执行安全重置时，才需要在客户端更新订阅地址。

5. **🎨 优化初始化表单与响应式布局**
   - 初始化码、新密码、确认密码统一使用深色主题输入控件，并提供显示/隐藏密码按钮；
   - 修复首次引导中输入框宽高、间距与卡片高度不协调的问题，在桌面和较矮视窗中均可完整操作；
   - 保留键盘导航、无障碍名称、错误提示与提交中状态，密码不一致或验证码无效时给出明确反馈。

6. **📚 完善 README 部署与升级说明**
   - Docker Compose、Docker CLI、NAS 和 Windows 原生部署步骤统一改为面板首次设置流程；
   - 补充如何从容器日志查看验证码、旧版本升级行为、常见错误处理，以及修改/忘记密码的安全操作；
   - 明确项目没有默认密码，不建议复用 SSH、系统 `root` 密码或直播平台 Cookie；
   - Windows ZIP 继续由 GitHub Actions 云端构建并附加到 Release，本地不额外制作发布压缩包。

#### 升级提示

- **已配置管理员密码的用户**：直接升级即可，原密码、管理配置与播放令牌保持不变；
- **从无密码版本升级的用户**：升级并启动后访问面板，按引导创建密码；Docker / NAS 用户从本次容器启动日志复制最新验证码；
