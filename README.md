...
> [!IMPORTANT]
> **LiveStreamGateway 没有默认管理员密码。** 从 `v1.5.6` 起，首次部署，或从没有管理员密码的旧版本升级后，直接访问 Web 面板即可进入创建密码引导，不再需要手工调用 `/api/auth/setup`。

### Docker / NAS 首次设置

1. 启动容器后，在浏览器访问 `http://<NAS或主机IP>:9898`；
2. 面板会自动显示 **“首次使用 · 创建管理员密码”**，而不是普通登录框；
3. 打开 NAS / Docker 管理面板中的 `live-stream-gateway` 容器，进入 **日志（Logs）** 页面；
4. 找到并复制本次启动输出的初始化码：

   ```text
   [Security] 首次设置验证码: ABCD-EFGH-JKLM-NPQR
   ```

5. 在引导页输入初始化码，输入并确认一个 **12～256 个字符**的管理员密码；
6. 点击 **“创建密码并进入控制台”**。设置成功后会自动登录，并同时生成独立的 Jellyfin / IPTV 播放令牌。

如果更习惯使用命令行，也可以只用下面的命令**查看日志中的初始化码**，密码仍然在网页引导中设置：

Linux / NAS：

```bash
docker logs live-stream-gateway 2>&1 | grep '首次设置验证码' | tail -n 1
```

Windows PowerShell：

```powershell
docker logs live-stream-gateway 2>&1 | Select-String '首次设置验证码' | Select-Object -Last 1
```

> [!NOTE]
> 初始化码采用 16 位高熵随机字符并以四组显示，只在**本次进程启动且密码尚未创建**时有效。容器或程序重启会生成新码，请使用日志中最后一次输出的码；设置成功后该码立即作废。连续输错会触发限速。不要公开初始化码或把它提交到 GitHub。

### Windows 原生 EXE / 源码运行首次设置

启动 `LiveStreamGateway.exe` 或 `dotnet run` 后，在同一台电脑打开 `http://localhost:9898`。本机直连会自动显示创建密码引导，**不要求输入初始化码**；输入并确认新密码即可进入控制台。

如果通过另一台电脑、局域网 IP 或 HTTPS 反向代理访问，面板会要求输入本次启动服务日志中显示的一次性初始化码。原生运行时最简单的方式仍是在运行主机上通过 `localhost` 完成首次设置。

### 从旧版本升级

- `AdminPasswordHash` 不存在或为空时，升级后第一次访问自动进入创建密码引导；原有频道、Cookie、推流模式等配置不会被清空；
- 已经设置过管理员密码的实例仍显示普通登录界面，不会重新生成或要求初始化码；
- 首次设置会自动生成新的随机播放令牌。从不带播放令牌的旧版本升级后，请把管理端显示的新 M3U 地址同步到 Jellyfin / IPTV 客户端；
...
| 现象 | 原因与处理方法 |
| :--- | :--- |
| 看不到初始化码输入框 | 通过 `localhost` / `127.0.0.1` 本机直连时不需要初始化码，这是正常行为。 |
| 容器日志中有多个初始化码 | 容器曾在未完成设置时重启；只使用日志中最后一次输出的码。旧码已经失效。 |
| 提示“一次性初始化码无效” | 检查是否复制了最新码。连字符和大小写不会影响识别，但不要混入其它日志文字。 |
| 提示“初始化码尝试过于频繁” | 同一来源连续输错达到限制，请等待响应提示的时间后再试；也可以重启尚未初始化的容器以生成新码。 |
...
> [!WARNING]
> 忘记密码后的安全重置会生成新的随机播放令牌，旧 M3U、HLS 和分片地址会立即失效。重置成功后，必须把管理端显示的新 M3U 地址同步更新到 Jellyfin / IPTV 客户端。重置前务必备份 `config.json`，不要通过删除整个配置文件来重置密码。
...
> [!IMPORTANT]
> 容器启动后没有可供尝试的默认密码。请直接访问 `http://<NAS或主机IP>:9898`，按照[首次访问引导](#admin-password-setup)创建密码；需要的一次性初始化码可在容器日志中直接查看。
...
程序没有默认管理员密码。首次运行后访问 `http://localhost:9898`，面板会自动显示[首次设置引导](#admin-password-setup)；本机直连无需初始化码，输入并确认新密码后会自动登录。随后可在管理端复制受保护的 M3U 地址。
